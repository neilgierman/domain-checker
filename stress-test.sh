...
./wait-for-it.sh web:80 --timeout=60
echo "2. Both of these should return unknown"
if [ "`curl -s http://web/domains/example.com | jq -r .Status`" != "unknown" ]
then
   exit 1
fi
if [ "`curl -s http://web/domains/example2.com | jq -r .Status`" != "unknown" ]
then
   exit 1
fi
for i in `seq 1 499`
do
   curl -s -X PUT http://web/events/example.com/delivered &
   curl -s -X PUT http://web/events/example.com/delivered &
   curl -s -X PUT http://web/events/example2.com/delivered &
   curl -s -X PUT http://web/events/example2.com/delivered &
done
wait $(jobs -p)
sleep 10
...
if [ "`curl -s http://web/domains/example.com | jq -r .Status`" != "catch-all" ]
then
   exit 1
fi
if [ "`curl -s http://web/domains/example2.com | jq -r .Status`" != "unknown" ]
...
curl -s -X PUT http://web/events/example.com/bounced &
wait $(jobs -p)
sleep 4
echo "5. Now the first should be not catch-all and the second still unknown"
if [ "`curl -s http://web/domains/example.com | jq -r .Status`" != "not catch-all" ]
then
   exit 1
fi
if [ "`curl -s http://web/domains/example2.com | jq -r .Status`" != "unknown" ]
